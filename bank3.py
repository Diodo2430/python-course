    ...
    def sacar(self, valor):
        if valor > 0:
            if self._saldo + self.limite_cheque_especial >= valor:
                ...
                if self._saldo < 0:
                    print(f"Utilizando cheque especial. Saldo atual: R${self._saldo:.2f}")
                return True
            else:
...
class ContaPoupanca(ContaBancaria):
    ...
    def aplicar_juros(self):
        if self.taxa_juros > 0 and self._saldo > 0:
            ...
            self._saldo += juros
            print(f"Juros de R${juros:.2f} aplicados na conta {self.numero_conta}. Novo saldo: R${self._saldo:.2f}")
        else:
